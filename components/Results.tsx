...
function ScoreBox({ score, isWinner }: { score: number; isWinner: 'win' | 'draw' | 'lose' }) {
  ...
  return <div className={`${base} bg-white/5 border-2 dark:border-white/5 text-snow`}>{score}</div>
}
      ...
      <div className="flex flex-col gap-0">
        {results.map((r, i) => {
          ...
          return (
            ...
            >
              <ChevronDown className="text-mist" />
              <span className={`flex-1 text-xs md:text-sm ${hState === 'win' ? 'text-snow font-semibold' : 'text-mist'}`}>
                {r.home}
              </span>
              <div className="flex items-center gap-1 mx-2 md:mx-4">
                <ScoreBox score={r.homeScore} isWinner={hState} />
                <span className="text-snow text-xs mx-0.5">–</span>
                <ScoreBox score={r.awayScore} isWinner={aState} />
              </div>
              <span className={`flex-1 text-xs md:text-sm text-right ${aState === 'win' ? 'text-snow font-semibold' : 'text-mist'}`}>
                {r.away}
              </span>
            </div>
          )
