...
export default function FplPreview({ currentEvent, topPlayers, teams }: Props) {
  return (
    <div className="grid grid-cols-1 bg-white dark:bg-[#071033] border-b px-6 border-blue-500/20 dark:border-blue-500/10">
      <div className="max-w-6xl mx-auto w-full py-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900 dark:text-snow">
              {currentEvent?.name ?? "Gameweek"}
            </h3>
            {currentEvent?.deadline_time && (
              <p className="text-sm text-slate-600 dark:text-snow/40">
                Deadline:{" "}
          ...
          <div className="text-right">
            <p className="text-xs text-slate-600 dark:text-snow/30">
            ...
            <div className="mt-2 flex gap-3 items-center justify-end">
              {topPlayers.map((p) => (
                <div
                  key={p.id}
                  className="bg-gray-50 dark:bg-white/[0.03] px-3 py-2 rounded text-sm text-slate-900 dark:text-snow/90"
                >
                  <div className="font-semibold">{p.web_name}</div>
                  <div className="text-xs text-slate-600 dark:text-snow/40">
