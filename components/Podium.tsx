...
export default function Podium() {
    return (
        <div className="grid grid-cols-1 bg-white dark:bg-[#0f172a] border-b px-6 border-blue-500/10">
            <div className="max-w-6xl mx-auto w-full">
                {/* Fixture of the Week */}
                <div>
                    <div className="py-12">
                        <div>
                          <p className="text-xl md:text-3xl font-bold tracking-[0.2em] uppercase text-snow">
                            League Leaders
                          </p>
                          ...
                          <div className="grid md:grid-cols-[2fr_1.6fr_1.3fr] grid-cols-1 gap-6">
                            ...
                            <div className="flex items-center gap-3 md:h-36 bg-yellow-400/10 border border-yellow-400/20 rounded-xl px-4 py-7 hover:scale-[104%] transition-all ease-in-out duration-300">
                              <span className="text-4xl md:text-5xl font-black dark:text-yellow-300/30 text-yellow-300">2</span>
                              <div className="flex-1 min-w-0 ps-2">
                                ...
                                <p className="text-sm lg:text-md font-light text-mist mt-0.5 truncate">{leaders[1].manager}</p>
                              </div>
                              <span className="text-xl lg:text-2xl font-black text-yellow-300">{leaders[1].pts}</span>
                            </div>

                            {/* 3rd */}
                            <div className="flex items-center gap-3 md:h-28 dark:bg-white/[0.04] border dark:border-white/[0.07] rounded-xl px-4 py-4 hover:scale-[104%] transition-all ease-in-out duration-300">
                              <span className="text-3xl md:text-4xl font-black text-snow">3</span>
                              <div className="flex-1 min-w-0 ps-2">
                                <p className="text-md lg:text-2xl font-semibold text-snow leading-tight tracking-wider truncate">{leaders[2].team}</p>
                                <p className="text-xs lg:text-sm text-mist mt-0.5 truncate">{leaders[2].manager}</p>
                              </div>
                              <span className="text-md lg:text-xl font-black text-snow">{leaders[2].pts}</span>
                            </div>
                          </div>
                        </div>
                    </div>
